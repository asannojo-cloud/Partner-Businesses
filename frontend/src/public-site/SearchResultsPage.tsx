...
export default function SearchResultsPage() {
  ...
  }, [categoryParam, q, subCategory, healthCheck, memberDiscount, familyAvailable, sort]);
    ...
    <div className="p-4">
      ...
      {categoryDef && (
        <div className="flex flex-wrap gap-1.5 mb-3">
          <button onClick={() => setFilter("subCategory", null)} className={`text-xs px-2.5 py-1 rounded-full border ${!subCategory ? "bg-brand-900 text-white border-brand-900" : "bg-white border-slate-300 text-slate-600"}`}>전체</button>
          {categoryDef.subCategories.map((s) => (
            <button key={s} onClick={() => setFilter("subCategory", s)} className={`text-xs px-2.5 py-1 rounded-full border ${subCategory === s ? "bg-brand-900 text-white border-brand-900" : "bg-white border-slate-300 text-slate-600"}`}>{s}</button>
          ))}
        </div>
      )}
      ...
      {!categoryDef && (
        <div className="flex flex-wrap gap-1.5 mb-4">
          {CATEGORIES.map((c) => (
            <a key={c.code} href={`/category/${c.code}`} className="text-xs px-2.5 py-1 rounded-full border border-slate-300 bg-white text-slate-600">{c.label}</a>
          ))}
        </div>
      )}
